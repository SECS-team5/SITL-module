...
Этот документ проверяет актуальный pipeline компонентов new-SITL:

- `sitl_verifier` принимает raw-сообщения и публикует verified-сообщения
- `sitl_controller` обновляет состояние дрона в Redis
- `sitl_core` двигает дрон в Redis с частотой 10 Гц
- `sitl_messaging` отвечает текущей позицией в `reply_to`
...
```bash
docker compose logs -f sitl_verifier sitl_controller sitl_core sitl_messaging
```
...
```bash
cd components/sitl_verifier && make docker-up &
cd components/sitl_controller && make docker-up &
cd components/sitl_core && make docker-up &
cd components/sitl_messaging && make docker-up &
```

Или используйте единый docker-compose в корне:
...
- Zookeeper и Kafka в статусе `Up (healthy)`
- Redis в статусе `Up (healthy)`
- Все компоненты `sitl_*` в статусе `Up`

### Тест 1. HOME проходит через verifier и попадает в Redis

Терминал 1 (consumer verified-home):
...
Терминал 2 (producer sitl-drone-home):
...
- в логах `sitl_controller` есть сообщение о сохранении HOME
...
Отправить (отсутствует home_alt):
...
- в логах `sitl_verifier` есть reject по schema validation
...
- в логах `sitl_controller` есть сообщение об игнорировании команды без HOME
...
- в логах `sitl_messaging` есть сообщение о возврате позиции
...
- в логах `sitl_messaging` есть reject с текстом `state not found`
