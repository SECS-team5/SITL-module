...
|-----|-----------|---------|------------|
| Центр верификации команд | verifier + sitl.verified-commands | Проверка входных команд | Средняя (0.60) |
| Центр доверенной домашней позиции | sitl.verified-home | Контроль точки возврата, геозона | Средняя (0.60) |
| Центр политик маршрутизации | controller | Применение активных политик к потокам, оркестрация | Высокая (0.90) |
| Центр безопасной телеметрии | sitl.telemetry.request + response | Запрос/ответ с контролем целостности | Высокая (0.85) |
...
|--------|--------------|------------------------|----------------|
| Chain of Responsibility | verifier → verified-commands | Цепочка проверок: аутентификация → валидация → белый список → подпись | Средняя (0.60) |
| Memento | sitl.verified-home | Криптографический снимок безопасной home-позиции, откат | Средняя (0.60) |
| Strategy | controller | Алгоритмы решений (политики) подменяются на лету | Высокая (0.90) |
| Proxy | sitl.telemetry.request/response | Перехват запросов телеметрии, добавление контроля целостности | Высокая (0.85) |
| Broker | messaging | Единая шина между компонентами (команды, телеметрия, события) | Низкая (0.20) |
